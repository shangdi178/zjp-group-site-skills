...
| 学历类别 | 中文页面 | 英文页面 |
|---------|---------|---------|
| 在校学生（博士+硕士） | 在校学生.cshtml | 在校学生-英文.cshtml |
| 毕业生 | 毕业生.cshtml | 毕业生-英文.cshtml |

注意：博士研究生和硕士研究生已合并为「在校学生」，导航从5项减为4项（课题组负责人 / 教师队伍 / 在校学生 / 毕业生）。对应的 CMS 节点标识为 `zxxs` / `en-zxxs`。
...
- 必须同时更新中英文两个页面
- 覆盖上传只需替换文件，不改模板
- **同一人不冲突原则**：同一个人可以同时在「在读博士」和「硕士毕业生」中出现（如先后在组里读硕读博）。源照片放在对应分类目录中，两张照片可以不同（硕士时期 vs 博士时期）。
- **毕业生去重**：处理新增毕业生时，检查 alumni/ 中是否已有该人。如有但属于不同阶段（如博士毕业生 vs 硕士毕业生），不跳过，两边都要展示。

### 教师个人页内容校验规则

更新教师个人页时，必须逐页对照 DOCX 源文件严格校验：

1. **内容精确匹配**：研究方向、邮箱、个人简介、教育经历、工作经历、项目、荣誉、论文、专利的文字必须与 DOCX 完全一致
2. **顺序一致**：每个章节内部的条目顺序必须与 DOCX 完全一致（如工作经历的时间顺序）
3. **空章节去除**：DOCX 中某个章节标题后无内容的（如"荣誉奖励："空行），该章节不要在页面上显示
4. **论文专利格式**：使用 `<div class="pi-pub-list"><p><strong>N.</strong> ...</p></div>` 格式
5. **专利章节位置**：专利章节必须在 `pi-cv-content` div 内部，不能放在外面
6. **数据源预处理**：每次更新前，用 DocWenCLI 或 python-docx 将 DOCX 转成 MD 保存到 `老师简介/md/` 目录，方便逐条对比
7. **逐页手动检查**：不要用 Python 脚本批量替换，逐个文件打开检查，确保不漏

### 论文/专利加粗格式

参考课题组负责人页（`课题组负责人.cshtml`）的格式：

```html
<p><strong>1.</strong> Authors, <strong>Teacher Name*</strong>. Title. <em>Journal Name Year</em>, Volume, Pages.</p>
```

规则：
1. **教师本人姓名** → `<strong>` 加粗（含 `*`、`#` 等上标符号）
2. **期刊名+年份** → `<em>` 斜体（不加粗）
3. **固定格式**：论文编号用 `<strong>N.</strong>`，专利同理

### PI 页（课题组负责人）特殊处理

- pi-meta-list 使用 2 列网格，长文本项（如研究方向）需加 `class="span-2"` 跨列
- 职务字段（副校长）保留在 meta-list 中
- 研究标签（pi-research-tags）不需要，直接移除

### CSS 注意事项

- `pi-meta-label`（如"研究方向"标签）：需加 `white-space:nowrap` 防止跨行
